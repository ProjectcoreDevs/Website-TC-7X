...
<body>
	...
	<div id='navigate-block'>
		<a class='navigate-item' href='index'>Home</a>-<a class='navigate-item' >Forum</a>
	</div>
	<div id='content'>
        <div id="forum-left">
			<?php $board = new BoardData; $forums = new ForumsData; $topic = new TopicsData; $board->construct(); $i=1;
			foreach($board->categorys as $category){ ?>
                <div id="forum-category-left">
					...
					<?php $forums->construct($i); $j=1;
					foreach ($forums->forums as $forum){
						$topic->construct_basic($j);
						if (!is_null($forums->forums[$j]['description'])) {?>
							<div class="forum-line">
								<a href="forum_list.php?id=<?php echo $forums->forums[$j]['id'] ?>/<?php echo urlencode($forums->forums[$j]['title']) ?>">
