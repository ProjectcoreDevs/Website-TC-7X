...
function bnet_encrypt($email, $password){
	$battlenet_password = strtoupper(bin2hex(strrev(hex2bin(strtoupper(hash("sha256",strtoupper(hash("sha256", strtoupper($email)).":".strtoupper($password))))))));
	return $battlenet_password;
}
function get_reg_errors($type){
	...
	for ($i=0;$i<count($errors);$i++){
		switch ($errors[$i]){
			...
			case "F": echo "<div id='notify'>Passwords does not match</div>";break;
		}
	}
}
...
function register_bnet_user($email,$bnet_password){
	global $DB_HOST,$DB_USERNAME,$DB_PASSWORD,$DB_AUTH;
	$con = connect($DB_HOST,$DB_USERNAME,$DB_PASSWORD);
	$sql = "INSERT INTO ".$DB_AUTH.".battlenet_accounts (email, sha_pass_hash) VALUES (?,?)";
	if ($stmt = $con->prepare($sql)) {
